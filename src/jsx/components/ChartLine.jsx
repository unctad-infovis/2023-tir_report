...
function LineChart({
  allow_decimals = false, data, idx, line_width = 3, note = false, show_first_label = true, source, subtitle = false, suffix = ['USD', 'USD', 'tonnes'], title
}) {
      ...
      chart: {
        events: {
          load() {
            ...
            this.renderer.image(`${window.location.href.includes('unctad.org') ? 'https://storage.unctad.org/2023-tir_report/' : './'}assets/img/unctad_logo.svg`, 5, 15, 80, 100).add();
          }
        },
        height: chartHeight,
        marginRight: 80,
